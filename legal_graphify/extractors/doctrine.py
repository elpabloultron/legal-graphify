...
def extract_doctrine_directory(doctrine_dir: str) -> nx.DiGraph:
    ...
    for root, _, files in os.walk(doctrine_dir):
        ...
        for file in sorted(files):
            ...
            obra = obra_match.group(1).strip() if obra_match else file.replace(".md", "")
            ...
            for sec in secciones:
                ...
                # Concordancias
                concordancias = re.findall(r"(?:Arts?\.?\s*\d+(?:\s*(?:bis|ter|quater))?(?:\s*(?:inc\.?\s*\d+|N°\s*\d+))*\s*(?:del\s*)?(?:CC|CPC|CPP|CP|COT|CT|CPR|Ley\s*\d+[\.\d]*))", sec_clean)
                for norm_text in set(concordancias):
                ...
                # Fallos
                fallos = re.findall(r"Rol\s*N°?\s*\d+[\.\d]*-\d{4}", sec_clean)
                for f_text in set(fallos):
